...
def loadData(filename, min_msPlayed, delimiter=";"):
  df = pd.read_csv(filename, delimiter=delimiter, encoding="UTF-8") #"ISO-8859-1")  
  numLines = df.shape[0]
...
def recommenderPoolSongs(songs, numFinalSongs, retryLimit=3):
	...
	# limite recommender spotify = 100 canzoni => cambio poolsize per essere sicuro di non oltrepassare questo limite 
	if len(songs) < 15:
		poolSize = 4
	else:
		poolSize = 5
	...
	for pool in pools:
		...
		while token and retry < retryLimit:
			try:
				...
				time.sleep(0.4)
				results = sp.recommendations(seed_tracks=pool,limit=numFinalSongs // len(pools))
				...
				print ("recommenderPoolSongs()- Retry #" + str(retry) + " limit: " + str(numFinalSongs // len(pools)) + " problem:", pool)
				retry += 1
				time.sleep(5)
	
	#print("recommended_pool (BEFORE): ", len(recommended_pool))
